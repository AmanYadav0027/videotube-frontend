...
const navStyles = `
  ...
  .navbar-header::before {
    ...
    background: linear-linear(120deg, rgba(99,102,241,0.08) 0%, rgba(236,72,153,0.06) 40%, rgba(16,185,129,0.05) 100%);
  ...
  .navbar-header::after {
    ...
    width: 60%; height: 1px;
    background: linear-linear(90deg, transparent, rgba(99,102,241,0.8), rgba(236,72,153,0.6), transparent);
  ...
  .navbar-logo {
    ...
    shrink:0; position:relative; z-index:1; transition:opacity 0.2s;
  }
  .navbar-logo:hover{opacity:0.8}
  .navbar-logo-dot {
    width:8px;height:8px;border-radius:50%;
    background:linear-linear(135deg,#6366f1,#ec4899);
    box-shadow:0 0 10px rgba(99,102,241,0.7);
    animation:pulseDot 2.4s ease-in-out infinite;shrink:0;
  }
  ...
  .navbar-links{display:flex;align-items:center;gap:4px;position:relative;z-index:1;shrink:0}
  ...
  .navbar-link::before{content:'';position:absolute;inset:0;border-radius:8px;background:linear-linear(135deg,rgba(99,102,241,0.15),rgba(236,72,153,0.1));opacity:0;transition:opacity 0.2s}
  .navbar-link::after{content:'';position:absolute;bottom:4px;left:12px;right:12px;height:1.5px;background:linear-linear(90deg,#6366f1,#ec4899);transform:scaleX(0);transform-origin:left;border-radius:2px;transition:transform 0.25s cubic-bezier(0.4,0,0.2,1)}
  ...
  .navbar-avatar-pill{
    ...
    text-decoration:none;margin-left:4px;shrink:0;transition:border-color 0.2s,background 0.2s;
  }
  .navbar-avatar-pill:hover{border-color:rgba(99,102,241,0.4);background:rgba(99,102,241,0.08)}
  .navbar-avatar-img{width:28px;height:28px;border-radius:50%;background:linear-linear(135deg,#6366f1,#ec4899);display:flex;align-items:center;justify-content:center;overflow:hidden;shrink:0}
  ...
  .navbar-hamburger-label{
    ...
    position:relative;z-index:1;shrink:0;transition:background 0.2s,border-color 0.2s,color 0.2s;
  }
  .navbar-hamburger-label:hover{background:rgba(99,102,241,0.15);border-color:rgba(99,102,241,0.35);color:#fff}
  .navbar-hamburger-label svg{width:18px;height:18px}
  ...
  .navbar-toggle:checked ~ .navbar-drawer{max-height:460px;opacity:1}
  ...
  .navbar-drawer-dot{width:6px;height:6px;border-radius:50%;background:linear-linear(135deg,#6366f1,#ec4899);opacity:0.5;shrink:0;transition:opacity 0.18s}
  ...
  .navbar-drawer-profile-avatar{width:34px;height:34px;border-radius:50%;background:linear-linear(135deg,#6366f1,#ec4899);display:flex;align-items:center;justify-content:center;overflow:hidden;shrink:0}
  ...
  .navbar-drawer-signin{display:flex;align-items:center;justify-content:center;margin-top:8px;padding:13px 14px;border-radius:10px;font-size:0.9375rem;font-weight:600;color:#fff;text-decoration:none;background:linear-linear(135deg,#6366f1,#7c3aed);transition:opacity 0.2s}
  .navbar-drawer-signin:hover{opacity:0.85}

  @media(max-width:860px){.navbar-search{max-width:260px}}
  @media(max-width:600px){.navbar-search{display:none}.navbar-links{display:none}.navbar-hamburger-label{display:flex}}
`;
...
export default function Navbar() {
  ...
  // 1. Use React Router's built-in hook instead of location.search
  const [searchParams] = useSearchParams();
  const queryFromUrl = searchParams.get("q") || "";

  // 2. Initialize the state directly from the URL
  const [searchInput, setSearchInput] = useState(queryFromUrl);

  // 3. Sync state only when the URL actually changes
  useEffect(() => {
    ...
    <div className="navbar-root">
      ...
      <header className="navbar-header">
        ...
        <nav className="navbar-links">
          <Link to="/" className={`navbar-link ${atHome ? "active" : ""}`}>
            Home
          </Link>
          <Link
            to="/about"
            className={`navbar-link ${atAbout ? "active" : ""}`}
          >
            About
          </Link>
          ...
          {isAuthenticated ? (
            <Link
              to="/profile"
              className="navbar-avatar-pill"
              aria-label="Your profile"
            >
              <span className="navbar-avatar-name">
                {user?.fullName || user?.username}
              </span>
              <span className="navbar-avatar-img">
                {user?.avatar ? (
        ...
        <label
          htmlFor="nav-toggle"
          className="navbar-hamburger-label"
      ...
      <div className="navbar-drawer">
        <div className="navbar-drawer-inner">
          ...
          <Link to="/about" className="navbar-drawer-link">
            <span className="navbar-drawer-dot" />
            About
          </Link>
          <hr className="navbar-drawer-divider" />
          ...
          {isAuthenticated ? (
            ...
            <Link to="/login" className="navbar-drawer-signin">
